...
## Recent decisions
- 2026-06-02 — Header fallback bug found on `capsule-sarl-avril-2026.dsn`: when `S21.G00.11.008` is absent, the UI title must not use the establishment SIRET before a readable name. `renderHeader` now falls back `establishment name → company/file name → establishment SIRET`, while the second header line still shows the corrected establishment SIRET.
